...
const CartProduct = ({cartInfo, proQuant, status, id,quantity, image, name, description, price, temp_id, category}) => {
   let singleProductPrice=quantity*price
    ...
    const remove_item = (e) =>{
        ...
        axios.delete("http://127.0.0.1:8000/api/delete-product-cart/"+id).then((response)=>{
  ...
  return (
        <div className=" w-200 md:w-1/2 relative flex flex-wrap items-center p-4 mb-4 border rounded-md bg-gray-200 dark:bg-gray-800 border-opacity-40">
          ...
          <div className="flex flex-col justify-between w-full px-4 md:w-56">
            <div className="mb-4">
              <a className="block text-lg font-medium hover:underline dark:text-gray-400" href="#">
                {name}
              </a>
            ...
            <div className="flex items-center justify-between">
              <span className="text-lg font-medium text-gray-800">Items: {quantity}</span>


            </div>
            <span className="text-l font-medium text-gray-800">Per Product: {price}</span>
