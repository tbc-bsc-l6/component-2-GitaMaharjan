...
const Product = () => {
  ...
  const deleteProduct = (e) => {
    ...
    axios
      .delete("http://127.0.0.1:8000/api/delete-products/" + id)
      .then((response) => {
    ...
    <div>
      <div className="flex md:flex-row flex-col items-baseline justify-between w-full mb-6">
        ...
        <div className="flex md:flex-row flex-col md:mt-0 mt-4 items-baseline justify-between gap-5">
          <Link to="/dashboard/products-form">
            <button
              type="submit"
              className=" relative overflow-x-auto shadow-md sm:rounded-lg  w-full bg-pink text-black py-2 px-4 rounded hover:bg-pink-200"
            >
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg ">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            <tr className={productItem.length === 0 ? "block" : "hidden"}>
            ...
            {productItem.map((item, id) => {
              return (
                <tr key={id}>
                  ...
                  <td className="px-6 py-4">{item.discount_id}</td>
